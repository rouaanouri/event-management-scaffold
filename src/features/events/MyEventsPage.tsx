...
const eventTypeLabels: Record<AttendedEventEntry["event"]["event_type"], string> = {
  CONFERENCE: "مؤتمر",
  WEBINAR: "ندوة عبر الإنترنت",
  WORKSHOP: "ورشة عمل",
};

const registrationStatusLabels: Record<AttendedEventEntry["registrationStatus"], string> = {
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
        <h1 className="mb-8 text-3xl font-extrabold text-white">فعالياتي</h1>

        {isLoading && (
          <p className="py-12 text-center text-sm text-white/40">جارٍ تحميل فعالياتك...</p>
        )}
        ...
        {!isLoading && !isError && entries && entries.length === 0 && (
          <div className="rounded-2xl border border-surface-border bg-surface-card p-8 text-center">
            <p className="mb-3 text-white/60">لم تسجّل في أي فعالية بعد.</p>
            <Link
              to="/"
            ...
            {entries.map((entry) => {
              ...
              return (
                ...
                >
                  <div className="min-w-0 flex-1">
                    <div className="mb-1 flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full bg-brand-500/15 px-2.5 py-0.5 text-xs font-semibold text-brand-300">
                        {eventTypeLabels[entry.event.event_type]}
                      </span>
                    </div>
                  ...
                  >
                    {registrationStatusLabels[entry.registrationStatus]}
                  </span>
                </Link>
              );
            })}
